...
| **Extrair Método** | `Curador::assinatura()` | Criação do método privado `_extrair_sobrenome()` |
...
### C. Extrair Método
* **Problema Original:** O método `assinatura()` continha um bloco complexo de estruturas condicionais para adivinhar a posição do sobrenome do autor.
* **Solução:** Esse trecho foi extraído para o método privado `_extrair_sobrenome()`, deixando o corpo do gerador de assinaturas mais limpo, curto e coeso.
